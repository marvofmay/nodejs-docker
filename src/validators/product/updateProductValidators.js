...
const isEANExistWithDifferentId = async (ean, { req }) => {
    const productId = req.params.id;
    const product = await ProductRepository.getProductByEanAndNotEqualId(ean, productId);
    if (product) {
        throw new Error('Product with this EAN exists');
    }
}
...
const updateProductValidator = [
    ...
    body('ean')
        ...
        .custom(isEAN)
        .custom(isEANExistWithDifferentId),
    body('price')
        .trim()
